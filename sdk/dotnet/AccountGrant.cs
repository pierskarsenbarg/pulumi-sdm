// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace PiersKarsenbarg.Sdm
{
    /// <summary>
    /// ## Import
    /// 
    /// AccountGrant can be imported using the id, e.g.,
    /// 
    /// ```sh
    ///  $ pulumi import sdm:index/accountGrant:AccountGrant example ag-12345678
    /// ```
    /// </summary>
    [SdmResourceType("sdm:index/accountGrant:AccountGrant")]
    public partial class AccountGrant : Pulumi.CustomResource
    {
        /// <summary>
        /// The account id of this AccountGrant.
        /// </summary>
        [Output("accountId")]
        public Output<string> AccountId { get; private set; } = null!;

        /// <summary>
        /// The resource id of this AccountGrant.
        /// </summary>
        [Output("resourceId")]
        public Output<string> ResourceId { get; private set; } = null!;


        /// <summary>
        /// Create a AccountGrant resource with the given unique name, arguments, and options.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resource</param>
        /// <param name="args">The arguments used to populate this resource's properties</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public AccountGrant(string name, AccountGrantArgs args, CustomResourceOptions? options = null)
            : base("sdm:index/accountGrant:AccountGrant", name, args ?? new AccountGrantArgs(), MakeResourceOptions(options, ""))
        {
        }

        private AccountGrant(string name, Input<string> id, AccountGrantState? state = null, CustomResourceOptions? options = null)
            : base("sdm:index/accountGrant:AccountGrant", name, state, MakeResourceOptions(options, id))
        {
        }

        private static CustomResourceOptions MakeResourceOptions(CustomResourceOptions? options, Input<string>? id)
        {
            var defaultOptions = new CustomResourceOptions
            {
                Version = Utilities.Version,
                PluginDownloadURL = "https://github.com/pierskarsenbarg/pulumi-sdm/releases/download/${VERSION}",
            };
            var merged = CustomResourceOptions.Merge(defaultOptions, options);
            // Override the ID if one was specified for consistency with other language SDKs.
            merged.Id = id ?? merged.Id;
            return merged;
        }
        /// <summary>
        /// Get an existing AccountGrant resource's state with the given name, ID, and optional extra
        /// properties used to qualify the lookup.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resulting resource.</param>
        /// <param name="id">The unique provider ID of the resource to lookup.</param>
        /// <param name="state">Any extra arguments used during the lookup.</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public static AccountGrant Get(string name, Input<string> id, AccountGrantState? state = null, CustomResourceOptions? options = null)
        {
            return new AccountGrant(name, id, state, options);
        }
    }

    public sealed class AccountGrantArgs : Pulumi.ResourceArgs
    {
        /// <summary>
        /// The account id of this AccountGrant.
        /// </summary>
        [Input("accountId", required: true)]
        public Input<string> AccountId { get; set; } = null!;

        /// <summary>
        /// The resource id of this AccountGrant.
        /// </summary>
        [Input("resourceId", required: true)]
        public Input<string> ResourceId { get; set; } = null!;

        public AccountGrantArgs()
        {
        }
    }

    public sealed class AccountGrantState : Pulumi.ResourceArgs
    {
        /// <summary>
        /// The account id of this AccountGrant.
        /// </summary>
        [Input("accountId")]
        public Input<string>? AccountId { get; set; }

        /// <summary>
        /// The resource id of this AccountGrant.
        /// </summary>
        [Input("resourceId")]
        public Input<string>? ResourceId { get; set; }

        public AccountGrantState()
        {
        }
    }
}
